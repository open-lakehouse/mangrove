...
///
// This adapter is per-file (schema evolution can shorten/reorder a file's physical schema) and
// does array-level reshape, so it cannot be folded into `ColumnMappingResolver`; it could reuse
// the resolver's leaf map as a scan-global baseline and only diff per file.
