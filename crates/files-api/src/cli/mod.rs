//! CLI surface for the standalone `files-api` binary.
//!
//! Two subcommands, mirroring the deployable half of the `server` crate's CLI
//! (the Files API has no database, so there is no `migrate`):
//!   - `serve` — run the Files API (see [`serve::serve`]). Flags overlay the
//!     config file, CLI flags taking precedence. A config file is optional; the
//!     `unity` backend then requires `--upstream-url`, `--backend memory` needs
//!     nothing.
...
use crate::cli::config::{AuthConfig, Backend, Config, DEFAULT_HOST, DEFAULT_PORT, UpstreamConfig};

/// `files-api` — standalone Unity Catalog volume Files API server.
#[derive(Debug, Parser)]
#[command(name = "files-api", version, about, long_about = None)]
pub struct Cli {
    #[command(subcommand)]
    pub command: Command,
}
...
/// The file store backend, as a CLI value.
...
impl From<BackendArg> for Backend {
    fn from(arg: BackendArg) -> Self {
        match arg {
            BackendArg::Unity => Backend::Unity,
...
/// the `unity` backend requires `--upstream-url`.
#[derive(Debug, Default, Clone, Args)]
pub struct ServeArgs {
    /// Config file path (YAML). Also read from `FILES_API_CONFIG`.
    #[arg(short, long, env = "FILES_API_CONFIG", value_name = "PATH")]
    ...
    /// File store backend. Overrides config; default `unity`.
    #[arg(long, value_enum)]
    pub backend: Option<BackendArg>,

    /// Upstream Unity Catalog base URL to vend through, e.g.
    /// `http://uc:8080/api/2.1/unity-catalog/`. Overrides config; required for the
    /// `unity` backend when no config file is supplied.
...
impl ServeArgs {
    /// Resolve the effective [`Config`]: load the file when given, else start from
    /// CLI-supplied values, then overlay the remaining flags and validate.
    pub fn resolve_config(&self) -> Result<Config, String> {
        let mut cfg = match &self.config {
            Some(path) => Config::load(path)?,
            None => {
                // No file: build from flags. The upstream block is created only
                // when a URL is supplied; `validate()` then enforces that the
                // unity backend has one.
                let upstream = self.upstream_url.clone().map(|base_url| UpstreamConfig {
                    base_url,
                    token: None,
                    region: None,
                });
    ...
    fn overlay(&self, cfg: &mut Config) {
        ...
        if let Some(url) = &self.upstream_url {
            match &mut cfg.upstream {
                Some(u) => u.base_url = url.clone(),
                None => {
                    cfg.upstream = Some(UpstreamConfig {
                        base_url: url.clone(),
                        token: None,
                        region: None,
                    })
...
pub struct HealthcheckArgs {
    /// Config file path used to resolve the probe target. Also `FILES_API_CONFIG`.
    #[arg(short, long, env = "FILES_API_CONFIG", value_name = "PATH")]
    pub config: Option<String>,
...
mod tests {
    ...
    #[test]
    fn serve_unity_requires_upstream_without_file() {
        let args = ServeArgs::default();
        assert!(args.resolve_config().is_err());
    }

    #[test]
    fn serve_memory_needs_no_upstream() {
        let args = ServeArgs {
            backend: Some(BackendArg::Memory),
            ..ServeArgs::default()
        };
    ...
    fn serve_flags_overlay_and_build_config() {
        ...
        let cfg = args.resolve_config().unwrap();
        let upstream = cfg.upstream.as_ref().unwrap();
        assert_eq!(upstream.base_url, "http://uc/");
        assert_eq!(
            upstream.token.as_ref().unwrap().value().as_deref(),
            Some("secret")
        );
    ...
    fn healthcheck_parses() {
        let cli = Cli::try_parse_from(["files-api", "healthcheck", "--port", "9000"]).unwrap();
    ...
    fn no_subcommand_is_an_error() {
        assert!(Cli::try_parse_from(["files-api"]).is_err());
    }
}
