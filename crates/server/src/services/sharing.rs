...
impl ServerHandler<RequestContext> {
    ...
    pub(super) async fn resolve_table_location(
        ...
        let Some(table_object) = share_info.objects.iter().find(|o| {
            o.shared_as.as_deref()
                == Some(format!("{}.{}", table_ref.schema, table_ref.table).as_str())
        }) else {
    ...
    pub(super) async fn resolve_volume_location(
        ...
        let Some(object) = share_info.objects.iter().find(|o| {
            o.shared_as.as_deref() == Some(shared_as.as_str())
                && matches!(
                    o.data_object_type.as_known(),
                    Some(DataObjectType::Volume | DataObjectType::AgentSkill)
                )
        }) else {
            return Err(Error::NotFound);
        };
...
fn to_sharing_credentials(cred: TemporaryCredential) -> Result<SharingTemporaryCredentials> {
    let credentials = match cred.credentials {
        Some(UcCredentials::AwsTempCredentials(c)) => {
            Some(SharingCredentials::from(SharingAwsCredentials {
        ...
        Some(UcCredentials::AzureUserDelegationSas(c)) => {
            Some(SharingCredentials::from(SharingAzureUserDelegationSas {
                sas_token: c.sas_token,
                ..Default::default()
            }))
        }
        Some(UcCredentials::GcpOauthToken(c)) => {
            Some(SharingCredentials::from(SharingGcpOauthToken {
                oauth_token: c.oauth_token,
                ..Default::default()
            }))
        }
        Some(UcCredentials::R2TempCredentials(c)) => {
            Some(SharingCredentials::from(SharingR2Credentials {
                access_key_id: c.access_key_id,
                secret_access_key: c.secret_access_key,
                session_token: c.session_token,
                ..Default::default()
...
impl ServerHandler<RequestContext> {
    ...
    async fn shared_assets(
        ...
        let items = share_info
            .objects
            .iter()
            .filter(|o| o.data_object_type.as_known() == Some(kind))
            .filter_map(|o| {
                let (schema, name) = o.shared_as.as_deref()?.split_once('.')?;
...
mod tests {
    ...
    fn maps_aws_credentials_preserving_fields() {
        let cred = uc_cred(Some(UcCredentials::from(AwsTemporaryCredentials {
    ...
    fn maps_gcp_credentials() {
        let cred = uc_cred(Some(UcCredentials::from(GcpOauthToken {
    ...
    fn azure_aad_and_missing_credentials_are_unsupported() {
        ...
        assert!(
            to_sharing_credentials(uc_cred(Some(UcCredentials::from(AzureAad {
